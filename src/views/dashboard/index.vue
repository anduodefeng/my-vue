  ...
  <div class="dashboard-container">
    <div style="width: 1400px; height: 50px">
      ...
      <div class="assetsDiv">
        ...
        <span v-if="totalProfit > 0" style="color: red" class="infoNum">{{
          totalProfit
        }}</span>
        <span v-if="totalProfit < 0" style="color: green" class="infoNum">{{
          totalProfit
        }}</span>
      ...
      <div class="assetsDiv">
        ...
        <span v-if="totalProfitRate > 0" style="color: red" class="infoNum"
          >{{ totalProfitRate }}%</span
        >
        <span v-if="totalProfitRate < 0" style="color: green" class="infoNum"
          >{{ totalProfitRate }}%</span
        >
        <span v-if="totalProfitRate == 0" class="infoNum">{{
          totalProfitRate
        }}</span>
      </div>
    </div>
    <div class="chartDiv">
      ...
      <div
        ref="totalLine"
        style="width: 790px; height: 400px; float: left"
      ></div>
    </div>
    <div class="chartDiv">
      <div
...
export default {
  ...
  methods: {
    ...
    initCharts() {
      ...
      const myRobustPortfolioCharts = this.$echarts.init(
        robustPortfolio,
        "walden"
      );
      const myAggressivePortfolioCharts = this.$echarts.init(
        aggressivePortfolio,
        "walden"
      );
      const profitEveryWeekCharts = this.$echarts.init(
        profitEveryWeek,
        "walden"
      );
      const profitRateEveryWeekCharts = this.$echarts.init(
        profitRateEveryWeek,
        "walden"
      );
